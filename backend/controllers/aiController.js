...
export async function getProductAIAnalysis(req, res) {
  try {
    ...
    if (req.query.phoneBrandId) {
      filters += ' AND dm.family = ?';
      params.push(req.query.phoneBrandId);
    }
    ...
    const availableDays = range?.first_sale_date
      ? Math.max(1, Math.floor((Date.now() - new Date(range.first_sale_date).getTime()) / 86400000) + 1)
      : 0;
...
export async function getSalesOpportunities(req, res) {
  try {
    ...
    const [summary, pairs, products] = await Promise.all([
      ...
      query(
        `SELECT a.product_id AS product_a_id, b.product_id AS product_b_id, COUNT(DISTINCT a.order_id) AS together_orders
         FROM order_items a
      ...
      query(
        `SELECT p.id, p.name, p.sku, p.stock_quantity, p.min_stock, p.price, p.cost_price,
         ...
         LEFT JOIN orders o ON o.id = oi.order_id
         WHERE p.is_active = 1
         GROUP BY p.id, p.name, p.sku, p.stock_quantity, p.min_stock, p.price, p.cost_price, c.name, dm.family`,
...
export async function generateAISalesOpportunities(req, res) {
  try {
    ...
    if (!candidates.length) {
      return res.status(400).json({ message: 'Không có ứng viên phù hợp để AI phân tích' });
    }

    res.json(await generateSalesStrategies({ days, orderCount, candidates }));
  } catch (error) {
    res.status(502).json({
      message: 'Hugging Face chưa thể tạo chiến lược; hệ thống sẽ dùng phân tích dự phòng',
      error: error.message
    });
  }
}
