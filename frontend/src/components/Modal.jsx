...
export default function Modal({ isOpen, onClose, title, children, maxWidth = 'max-w-2xl' }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex h-[100dvh] w-screen touch-none items-end justify-center overflow-hidden overscroll-none bg-black/60 sm:items-center sm:px-4 sm:py-6" role="presentation">
      <div className={`relative max-h-[94dvh] w-full ${maxWidth} touch-pan-y overscroll-contain overflow-y-auto bg-white p-4 shadow-xl sm:max-h-[90vh] sm:p-6`} role="dialog" aria-modal="true" aria-label={title} onClick={(event) => event.stopPropagation()}>
        <div className="sticky -top-4 z-30 -mx-4 mb-5 flex items-center justify-between gap-4 border-b bg-white px-4 py-3 sm:static sm:mx-0 sm:border-0 sm:p-0">
