...
export default function RestockSuggestionTab({ analysis }) {
  ...
  const openPurchaseOrder = (rows) => {
    const actionableRows = rows.filter((item) => item.suggestedQuantity > 0);
    if (!actionableRows.length) return toast.error('Hãy chọn ít nhất một sản phẩm có đề xuất nhập.');
    navigate('/purchase-orders', { state: { suggestedItems: actionableRows.map((item) => ({ product_id: item.productId, quantity: item.suggestedQuantity, import_price: item.hasValidCostPrice ? item.costPrice : 0 })), source: 'restock-suggestions' } });
  };
