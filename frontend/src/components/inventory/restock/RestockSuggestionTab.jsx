...
const statusMeta = {
  urgent: { label: 'Cần nhập gấp', className: 'bg-red-50 text-red-700 ring-red-600/10' },
  reorder: { label: 'Nên nhập', className: 'bg-amber-50 text-amber-700 ring-amber-600/10' },
  watch: { label: 'Theo dõi', className: 'bg-sky-50 text-sky-700 ring-sky-600/10' },
  enough: { label: 'Đủ hàng', className: 'bg-emerald-50 text-emerald-700 ring-emerald-600/10' }
};

function getStatus(item) {
  if (Number(item.suggestedQuantity) <= 0) return statusMeta.enough;
  const days = Number(item.daysCover);
  if (Number.isFinite(days) && days <= 7) return statusMeta.urgent;
  if (Number.isFinite(days) && days <= 15) return statusMeta.reorder;
  return statusMeta.watch;
}
...
function suggestionReason(item) {
  const opening = `Sản phẩm hiện còn ${integer(item.currentStock)} cái. Trong 30 ngày gần nhất bán được ${integer(item.sold30Days)} cái, trung bình ${demandSubline(Number(item.daily30))}.`;
  const cover = item.daysCover === null ? 'chưa xác định được số ngày đủ bán' : `còn đủ bán khoảng ${Math.max(0, Math.round(item.daysCover))} ngày`;
  if (Number(item.suggestedQuantity) <= 0) return `${opening} Tồn kho hiện tại ${cover}, nên hiện tại chưa cần nhập thêm.`;
  return `${opening} Với tốc độ này, tồn kho hiện tại ${cover}. Hệ thống đề xuất nhập thêm ${integer(item.suggestedQuantity)} cái để đảm bảo đủ hàng trong 30 ngày tới.`;
}
...
export default function RestockSuggestionTab({ analysis }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: rows.length,
      high: rows.filter((item) => getStatus(item) === statusMeta.urgent).length,
  ...
  return <section className="space-y-4 bg-[#f8fafc] p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatCard icon={Boxes} label="Sản phẩm cần kiểm tra" value={stats.total.toLocaleString('vi-VN')} note="Có đề xuất nhập theo nhu cầu" /><StatCard icon={AlertTriangle} label="Sắp hết hàng" value={stats.high.toLocaleString('vi-VN')} note="Cần nhập trong thời gian sớm" tone="bg-red-50 text-red-700" /><StatCard icon={ShoppingCart} label="Tổng số cần nhập" value={stats.quantity.toLocaleString('vi-VN')} note="Cộng tất cả sản phẩm" tone="bg-cyan-50 text-cyan-700" /><StatCard icon={WalletCards} label="Dự kiến tiền nhập" value={formatCurrency(stats.cost)} note={stats.missingCost ? 'Chưa gồm sản phẩm thiếu giá nhập' : 'Theo giá nhập hiện tại'} tone="bg-emerald-50 text-emerald-700" /></div>
