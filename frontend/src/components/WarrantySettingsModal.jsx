...
function Table({heads,rows,minHeight}) { return <div className="overflow-x-auto border" style={minHeight ? { minHeight } : undefined}><table className="min-w-full table-fixed text-left text-sm"><thead className="bg-gray-50"><tr>{heads.map((x,i)=><th key={i} className="whitespace-nowrap px-3 py-3">{x}</th>)}</tr></thead><tbody className="divide-y">{rows.map((r,i)=><tr key={i}>{r.map((x,j)=><td key={j} className="min-w-32 px-3 py-3 text-gray-700">{x}</td>)}</tr>)}</tbody></table></div> }
