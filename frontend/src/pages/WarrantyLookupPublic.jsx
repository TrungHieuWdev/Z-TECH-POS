...
export default function WarrantyLookupPublic() {
  ...
  useEffect(() => {
    ...
    api.get(`/public/warranties/${encodeURIComponent(publicToken)}`)
      .then((response) => {
        if (active) setData(response.data);
      })
      .catch((requestError) => {
        if (!active) return;
        setError(requestError.response?.data?.message || requestError.message || 'Không thể tra cứu bảo hành');
      });
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-10 text-slate-900">
      <section className="mx-auto max-w-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-200 pb-5">
          ...
          <div>
            <h1 className="text-xl font-bold">Tra cứu bảo hành Z-TECH</h1>
            <p className="text-sm text-slate-500">Thông tin bảo hành công khai</p>
          </div>
        </div>
