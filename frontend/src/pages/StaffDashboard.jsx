...
export default function StaffDashboard() {
  ...
  return <div className="space-y-5"><div><h1 className="text-2xl font-bold">Tổng quan ca làm</h1><p className="mt-1 text-sm text-gray-500">Chỉ hiển thị số liệu bán hàng của chính bạn trong ngày hôm nay.</p></div><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({label,value,icon:Icon})=><article key={label} className="border bg-white p-4"><div className="flex items-start justify-between"><div><p className="text-sm text-gray-500">{label}</p><p className="mt-2 text-xl font-bold">{value}</p></div><div className="grid h-9 w-9 place-items-center bg-brand-soft text-brand-strong"><Icon size={18}/></div></div></article>)}</section></div>;
}
